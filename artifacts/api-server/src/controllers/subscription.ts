import { Request, Response } from "express";
import StudioPlan from "../models/StudioPlan.js";
import StudioSubscription from "../models/StudioSubscription.js";
import { logger } from "../lib/logger.js";

export const getPlans = async (req: Request, res: Response) => {
  try {
    const plans = await StudioPlan.find({ isActive: true }).sort({ pricePaise: 1 });
    res.json(plans);
  } catch (error) {
    logger.error({ error }, "getPlans error");
    res.status(500).json({ error: "Failed to fetch plans" });
  }
};

export const createPlan = async (req: Request, res: Response) => {
  try {
    const { name, description, pricePaise, intervalDays, curatedItems } = req.body;
    const plan = await StudioPlan.create({ name, description, pricePaise, intervalDays, curatedItems });
    res.status(201).json(plan);
  } catch (error) {
    logger.error({ error }, "createPlan error");
    res.status(500).json({ error: "Failed to create plan" });
  }
};

export const createSubscription = async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const { planId, deliveryDay, deliveryAddress, nextDeliveryAt } = req.body;

    const existing = await StudioSubscription.findOne({ userId, status: "active" });
    if (existing) {
      res.status(409).json({ error: "Already have an active subscription" });
      return;
    }

    const sub = await StudioSubscription.create({
      userId,
      planId,
      deliveryDay,
      deliveryAddress,
      nextDeliveryAt: new Date(nextDeliveryAt),
    });
    res.status(201).json(sub);
  } catch (error) {
    logger.error({ error }, "createSubscription error");
    res.status(500).json({ error: "Failed to create subscription" });
  }
};

export const getMySubscription = async (req: Request, res: Response) => {
  try {
    const userId = req.user!.id;
    const sub = await StudioSubscription.findOne({ userId }).populate("planId").sort({ createdAt: -1 });
    if (!sub) {
      res.status(404).json({ error: "No subscription found" });
      return;
    }
    res.json(sub);
  } catch (error) {
    logger.error({ error }, "getMySubscription error");
    res.status(500).json({ error: "Failed to fetch subscription" });
  }
};

export const updateSubscriptionStatus = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { status } = req.body;
    const userId = req.user!.id;

    const sub = await StudioSubscription.findById(id);
    if (!sub) {
      res.status(404).json({ error: "Subscription not found" });
      return;
    }

    if (req.user!.role !== "admin" && sub.userId.toString() !== userId) {
      res.status(403).json({ error: "Forbidden: You do not own this subscription" });
      return;
    }

    sub.status = status;
    await sub.save();
    res.json(sub);
  } catch (error) {
    logger.error({ error }, "updateSubscriptionStatus error");
    res.status(500).json({ error: "Failed to update subscription" });
  }
};
